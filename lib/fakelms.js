	...
	switch (parts[1]) {
		case 'interactions':
			...
			switch (parts[2]) {
				...
				case '_children':
					return FakeLMSAPI.SUPPORTED_INTERACTIONS_FIELDS;
				default:
					// must be an integer
					...
					var field = parts[3];
					if (-1 == FakeLMSAPI.SUPPORTED_INTERACTIONS_FIELDS.indexOf(field)) {
						return this._fail(FakeLMS.ERRCODES.UNIMPLEMENTED_DATA_MODEL_ELEMENT);
					}
...
*/
FakeLMSAPI.prototype.Commit = function() {
	if ("string" != typeof wtf || wtf.length) {
		return this._fail(FakeLMS.ERRCODES.GENERAL_ARGUMENT_ERROR);
	}
	return this._ok();
...
FakeLMSAPI.prototype.GetErrorString = function(errCode) {
	return FakeLMSAPI.ERRSTRINGS[errCode];
}
...
window.FakeLMS /* visibility for direct browser testing */ = module.exports = FakeLMS;
