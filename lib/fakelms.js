...
FakeLMSAPI.prototype.Terminate = function(wtf) {
  ...
  window.localStorage.setItem(
    'total_time',
    Number(window.localStorage.getItem('total_time')) + this.session_time
  )
  return this._ok()
}
...
FakeLMSAPI.prototype.GetValue = function(path) {
  ...
  switch (parts[1]) {
    case 'entry': // no break
      entryValue = window.localStorage.getItem('entry')
    ...
    case 'suspend_data':
      return window.localStorage.getItem('suspend_data')
    case 'location':
  ...
  switch (parts[1]) {
    case 'exit':
      ...
      if (-1 !== ['suspend', 'logout'].indexOf(value)) {
        window.localStorage.setItem('entry', 'resume') // lms mimic
      } else {
        window.localStorage.setItem('entry', '') // lms mimic
      }
    ...
    case 'suspend_data':
      ...
      window.localStorage.setItem('suspend_data', value)
      break
    case 'location':
